...
public class Zoom {
    ...
    private static void onKey(InputEvent event) {
        if (instance.isActive() && keybind. isKey(event.key) && mc.currentScreen == null) {
            if (event.action == GLFW.GLFW_PRESS) {
                mc.options.smoothCameraEnabled = cinematic.value();
                zoomedIn = true;
                targetFov = (float) (mc.options.getFov().getValue() / Math.max(1, scale.value()));
            } else if (event.action == GLFW.GLFW_RELEASE || mc.currentScreen != null) {
                zoomedIn = false;
                mc.options.smoothCameraEnabled = false; // unfortunately no revert to previous state
            }
        }
    }
}
