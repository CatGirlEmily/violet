        ...
        dispatcher.register(literal("rotate")
            .then(literal("set")
                .then(argument("yaw", FloatArgumentType.floatArg(-180, 180))
                    .executes(ctx -> {
                        mc.player.setYaw(FloatArgumentType.getFloat(ctx, "yaw"));
                        return SINGLE_SUCCESS;
                    })
                    .then(argument("pitch", FloatArgumentType.floatArg(-90, 90))
                        .executes(ctx -> {
                            mc.player.setYaw(FloatArgumentType.getFloat(ctx, "yaw"));
                            mc.player.setPitch(FloatArgumentType.getFloat(ctx, "pitch"));
                            return SINGLE_SUCCESS;
                        })
                    )
                )
            ...
            .then(literal("add")
                .then(argument("yaw", FloatArgumentType.floatArg(-180, 180))
                    .executes(ctx -> {
                        mc.player.setYaw(MathHelper.wrapDegrees(mc.player.getYaw() + FloatArgumentType.getFloat(ctx, "yaw")));
                        return SINGLE_SUCCESS;
                    })
                    .then(argument("pitch", FloatArgumentType.floatArg(-90, 90))
                        .executes(ctx -> {
                            mc.player.setYaw(MathHelper.wrapDegrees(mc.player.getYaw() + FloatArgumentType.getFloat(ctx, "yaw")));
                            float pitch = mc.player.getPitch() + FloatArgumentType.getFloat(ctx, "pitch");
